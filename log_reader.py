...
for inf_log in glob.glob(f"{LOG_DIR}/*_inf_log.*"):
    ...
    new_row = {'params': inf_log_df['model_name'].iloc[0].replace(f'{ONNX_MODELS_DIR}/', '').replace('.onnx', ''), 'mem': None, 'latency': inf_log_df['avg_latency'].iloc[0], 'power': None, 'energy': None, 'num_images': inf_log_df['num_images'].iloc[0]}
